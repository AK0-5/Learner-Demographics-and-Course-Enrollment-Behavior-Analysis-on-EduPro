...
def load_data():
    ...
    possible_paths = [
        os.path.join(current_dir, "merged_data.csv"),
        os.path.join(current_dir, "merged_data.csv"),  # Your actual file name
        os.path.join(current_dir, "..", "merged_data.csv"),
        "merged_data.csv"
    ]
    ...
    raise FileNotFoundError(f"merged_data.csv not found in {current_dir}")
...
try:
    ...
    st.error(f"❌ {str(e)}")
    st.info("📁 Please make sure 'merged_data.csv' is in the same folder as this script")
    st.stop()
